...
generate_shiny_app <- function(dir = NULL,
  ...
  if(tolower(storage)=="dropbox"){
    shinyApp <- shiny_app_code
  }else if(tolower(storage)=="onedrive"){
    shinyApp <- shiny_app_code_365
  }


  if(is.null(dir)){
    writeLines(shinyApp, paste0(name,".R"))
    ...
    if(create_token){
      dir.create("data")
      if(tolower(storage)=="dropbox"){
        dropbox_token(new_user = T)
      }else if(tolower(storage)=="onedrive"){
        onedrive_token(new_user = T)
      }
    }
    ...
    if(!dir.exists(dir)){
      ...
      writeLines(shinyApp, paste0(dir,"/",name,".R"))
      ...
      if(create_token){
        dir.create("data")
        if(tolower(storage)=="dropbox"){
          dropbox_token(dir = dir,new_user = T)
        }else if(tolower(storage)=="onedrive"){
          onedrive_token(dir = dir,new_user = T)
        }
      }
...
### drop.folder(Dropbox folder)       ###
# this token should be put in the same folder with app.R or on the Shiny server
tokenfile <- file.path('data', 'token_365.rds')
logfile <- 'log.csv'
drop.folder <- 'folder'   # folder for keeping all the data files
token_365 <- readRDS(tokenfile)
od <- get_personal_onedrive(token = token_365)

# credential file should be save in a dropbox folder.
# get credentials list
credentials <- as.data.frame(od$load_dataframe(file.path(drop.folder, 'Credentials.csv')))
...
addLog <- function(user,site,file = file.path(drop.folder,logfile)){
  ...
  tmplog <- rbind(data.frame(login=user,timestamp=time.stamp), as.data.frame(od$load_dataframe(file)))

  write.csv(tmplog, file=logfile,row.names = F)

  od$upload_file(logfile,file.path(drop.folder, logfile) )
  # drop_upload(logfile, path = drop.folder)
...
addPID <- function(site, screenID, age, sex, randID, symptomDay, vaccine, treatment, file, timestamp, user){
  ...
  tmpdat <- rbind(as.data.frame(od$load_dataframe(file.path(drop.folder, file))),
                  data.frame(site = site,
                             screeningID = screenID,
                             age = age, sex = sex,
                             symptomDay=symptomDay,
  ...
  od$upload_file(file,file.path(drop.folder, file) )
...
existQ <- function(site, pid, age, sex){
  data.file <- paste0('data-',site,'.csv')
  tmp.dat <- as.data.frame(od$load_dataframe(file.path(drop.folder,data.file)))
  tmp <- tmp.dat[tmp.dat$screeningID == pid & tmp.dat$age == age & tmp.dat$sex == sex ,]
  tmp2 <- tmp.dat[tmp.dat$screeningID == pid & tmp.dat$age != age | tmp.dat$sex != sex ,]
  outlist <- list()
  if(nrow(tmp)==1){
...
getTreatment <- function(site, PID){
  ...
  # how many patients have been added?
  tmp.data <- as.data.frame(od$load_dataframe(file.path(drop.folder,data.file)))
  is.assigned <- PID %in% tmp.data[,2]
  output$Assigned <- is.assigned

  npid <- nrow(tmp.data)
  tmp.rand <- as.data.frame(od$load_dataframe(file.path(drop.folder,rand.file)))
  n.rand <- nrow(tmp.rand)
